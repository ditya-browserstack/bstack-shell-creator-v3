#!/usr/bin/env bash
# Install or UPDATE bstack-shell-creator-v3 into ~/.claude/skills/.
# Prefers git (so future updates are a fast git pull); falls back to unzipping a .skill bundle.
set -euo pipefail
DEST="$HOME/.claude/skills/bstack-shell-creator-v3"
...
echo "done. Restart Claude Code; the skill is 'bstack-shell-creator-v3'. Updates are offered automatically on use."
